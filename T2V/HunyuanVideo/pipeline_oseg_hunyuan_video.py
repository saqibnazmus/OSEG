...
def forward_with_stg(self, *args, **kwargs):
    # Extract the required arguments
    hidden_states = kwargs.get('hidden_states', args[0] if args else None)
    encoder_hidden_states = kwargs.get('encoder_hidden_states', args[1] if len(args) > 1 else None)
    return hidden_states, encoder_hidden_states
    
def forward_without_stg(self, *args, **kwargs):
    # Extract required arguments with fallbacks
    hidden_states = kwargs.get('hidden_states', args[0] if args else None)
    encoder_hidden_states = kwargs.get('encoder_hidden_states', args[1] if len(args) > 1 else None)
    temb = kwargs.get('temb', kwargs.get('timestep', args[2] if len(args) > 2 else None))
    attention_mask = kwargs.get('attention_mask', args[3] if len(args) > 3 else None)
    freqs_cis = kwargs.get('freqs_cis', kwargs.get('image_rotary_emb', args[4] if len(args) > 4 else None))
