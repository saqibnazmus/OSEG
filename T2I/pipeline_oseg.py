# Implementation of StableDiffusionXLOSEGPipeline
...
def orthogonalize_noise(cond_noise: torch.Tensor, uncond_noise: torch.Tensor) -> torch.Tensor:
    ...
    cond_flat = cond_noise.view(cond_noise.size(0), -1)  # [1, H*W]
    uncond_flat = uncond_noise.view(uncond_noise.size(0), -1)
    ...
    uncond_perp = uncond_perp_flat.view(uncond_noise.shape)
...
def spectral_norm(stensor, power_iterations=1):
    """
    Apply spectral normalization to a tensor of shape [B, N, C].
    Normalizes each [N, C] matrix in the batch.
    
    Args:
        tensor (torch.Tensor): Input tensor of shape [B, N, C]
        power_iterations (int): Number of power iterations for computing max singular value
    
    Returns:
        torch.Tensor: Spectrally normalized tensor of same shape
    """
    B, N, C = stensor.shape
    normalized_tensor = torch.zeros_like(stensor)
    
    for b in range(B):
        # Reshape [N, C] matrix for batch element b
        matrix = stensor[b]  # Shape: [N, C]
        
        # Initialize random vector for power iteration
        u = torch.randn(N, 1, device=stensor.device).to(stensor.dtype)
        v = torch.randn(C, 1, device=stensor.device).to(stensor.dtype)
        
        # Power iteration to approximate largest singular value
        for _ in range(power_iterations):
            v = nn.functional.normalize(torch.matmul(matrix.t(), u), dim=0)
            u = nn.functional.normalize(torch.matmul(matrix, v), dim=0)
        
        # Compute spectral norm (largest singular value)
        sigma = torch.matmul(torch.matmul(u.t(), matrix), v).abs()
        
        # Normalize the matrix
        normalized_tensor[b] = matrix / (sigma + 1e-8)  # Add epsilon for numerical stability
    
    return normalized_tensor.to(stensor.dtype)
...
def project_query_to_nullspace(query, eps=1e-6):
    """
    Project each query head in the batch onto its nullspace.
    
    Args:
        query: Query tensor of shape [batch_size, num_heads, seq_len, head_dim]
        eps: Threshold for singular values to be considered "zero"
    
    Returns:
        query_null: Projection onto nullspace, same shape as input
    """
    batch_size, num_heads, seq_len, head_dim = query.shape
    query_null = torch.zeros_like(query)

    for b in range(batch_size):
        for h in range(num_heads):
            # Extract the query matrix for this batch and head: [seq_len, head_dim]
            Q_head = query[b, h]

            # Convert to float32 if half precision on CUDA
            if Q_head.is_cuda and Q_head.dtype == torch.float16:
                Q_head = Q_head.float()

            # Compute SVD
            U, S, Vh = torch.linalg.svd(Q_head, full_matrices=True)

            # Identify nullspace basis (right singular vectors for zero singular values)
            null_mask = S < eps
            null_basis = Vh.T[:, null_mask]  # [head_dim, k], where k is nullity

            if null_basis.shape[1] > 0:
                # Project Q_head onto nullspace
                proj = null_basis @ torch.linalg.pinv(null_basis.T @ null_basis) @ null_basis.T @ Q_head
                query_null[b, h] = proj.to(dtype=query.dtype)
            else:
                query_null[b, h] = torch.zeros_like(Q_head, dtype=query.dtype)
...
class OSEGCFGSelfAttnProcessor:
    r"""
    ...
    def __call__(
        ...
        **kwargs,
        ...
        #print(torch.var(key), torch.var(query))
        ...
        if self.do_cfg:
            ...
            query = torch.cat((query_uncond, query_org, query_ptbo), dim=0)
            query1 = torch.cat((query_uncond, query_org, query_ptbo), dim=0)
            td = query.shape[2]
            H = 64
            W = td//H
            op, oo = compute_orthogonal_parallel_components(query, key)
            oos = gaussian_blur_2d(oo,  kernel_size=5, sigma=1.0)
            ops = gaussian_blur_2d(op,  kernel_size=5, sigma=1.0)
            ops1, oos1 = compute_orthogonal_parallel_components(query, oos)
            query2 =  oos #+ ops1 #+ oqs
            lg = query2.shape[1]
            query3 = torch.mean(query2,1,True).repeat(1,lg,1,1)
            # hquery3 = query3- gaussian_blur_2d(query3,  kernel_size=5, sigma=1.0)
            # query3 = query3 + 1e-2*hquery3
            vd = 0 # akahne 0 dite hobe
            query[vd] = query3[vd]  
            vd = 2
        else:
        ...
        #query = F.scaled_dot_product_attention(query, query, query, attn_mask=attention_mask, dropout_p=0.0, is_causal=False,)
    
        hidden_states = F.scaled_dot_product_attention(query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False,)

        # hid = F.scaled_dot_product_attention(hidden_states, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False,)

        # w1 = -0.2
        # w2 = 1 - w1

        # hidden_states = hidden_states * w2 + hid * w1
        
        # weights, factors = parafac(hidden_states.to(torch.float32), rank=3, normalize_factors=True)

 
        # hidden_states_lr = tl.cp_to_tensor((weights, factors)).to(query.dtype)

        # vd = 0
        # hidden_states[vd] = hidden_states_lr[vd]  
#####################################   OSEG  ##########################################
...
class StableDiffusionXLOSEGPipeline(
    DiffusionPipeline,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if self.do_seg:
                    ...
                    replace_processor = OSEGCFGSelfAttnProcessor(blur_sigma=seg_blur_sigma, do_cfg=self.do_classifier_free_guidance)
